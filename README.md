...
현재 노드 네트워크는 두 개의 상위 계열을 제공합니다.

- `/#/horror-dopamine`: 공포도파민 채널계
- `/#/apps`: 직접 만든 앱을 소개하는 앱계(도화지, TimeRoots)

브라우저 언어를 기준으로 한국어, 영어, 일본어를 자동 선택하며 헤더의 언어
버튼으로 직접 변경할 수 있습니다. 지원하지 않는 언어의 기본값은 한국어입니다.
...
앱계의 제품은 `src/apps.ts`의 `productApps` 배열에 추가합니다.
